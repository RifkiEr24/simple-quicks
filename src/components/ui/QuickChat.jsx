          ...
          if (commentResponse.status === 200) {
            ...
            if (comments.length > 0) {
              ...
              comments.reverse().forEach((comment) => {
                const publishDate = formatDateString(
                  comment.publishDate,
                  "dateOnly"
                );
      ...
      if (response.status === 200) {
        if (
          !selectedPost.comments[
            formatDateString(response.data.publishDate, " dateOnly")
          ]
        ) {
          selectedPost.comments[
            formatDateString(response.data.publishDate, "dateOnly")
      ...
      if (response.status === 200) {
          ...
          data: posts.data.map((item) => {
            if (item.id === selectedPost.id) {
              item.comments = Object.keys(item.comments).reduce((acc, date) => {
                acc[date] = item.comments[date].filter(
        ...
        setSelectedPost({
          ...
          comments: Object.keys(selectedPost.comments).reduce((acc, date) => {
            acc[date] = selectedPost.comments[date].filter(
              (comment) => comment.id !== commentId
            );
            return acc;
          }, {}),
    ...
    <Dialog open={open} modal={false}>
      <DialogContent index={index} onClose={onClose} container={container}>
        <div className="flex flex-col text-sm font-normal h-full">
          {loading ? (
            <div className="flex flex-col gap-3 items-center justify-center text-center text-primary-3 h-full">
            ...
            <div className="py-[22px] px-[32px] overflow-y-auto">
              {posts.data.map((item) => (
                <div
                  key={item.id}
                  className="flex gap-[17px] py-[22px] border-b border-b-primary-3 cursor-pointer"
                ...
                >
                  <div className="flex relative justify-center min-w-[54px]">
                    ...
                    <div
                      className={clsx(
                        "w-[34px] h-[34px] rounded-full flex items-center justify-center bg-primary-1",
                  ...
                  <div className="flex flex-col">
                    <div className="flex gap-3 items-center">
                      <span className="text-primary-1 max-w-[415px] font-semibold text-sm">
                        {item.text}
                      </span>
                      <span className="text-xs self-start mt-1 text-nowrap">
                        {formatDateString(item.publishDate)}
                      </span>
                    </div>
                    {item.latestComment && (
                      <div className="flex flex-col text-xs text-primary-2 mb-2">
                        <span className="font-bold">
                          {item.latestComment.owner.firstName}{" "}
                          {item.latestComment.owner.lastName}:
                        </span>
                        <span>{item.latestComment.message}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {Object.keys(selectedPost.comments).map((date) => (
                  <div key={date} className="flex flex-col gap-[9px] py-2">
                    <div className="flex gap-3 items-center">
                      ...
                      <span className="text-xs text-primary-2 font-semibold flex-grow-[1]  text-nowrap">
                        {date}
                      </span>
                      <div className="h-[1px] w-full bg-primary-2" />
                    </div>
                    {selectedPost.comments[date].map((comment, index) => (
                      <div
                        ...
                        className={clsx(
                          "flex gap-[15px]",
                          comment.owner.id === "6666fc43f47c6f581caeb135" &&
                        ...
                        >
                          <span
                            className={clsx(
                              ...
                              comment.owner.id === "6666fc43f47c6f581caeb135"
                                ? "text-chats-2-foreground"
                                : "text-chats-1-foreground"
                            )}
                          >
                            {comment.owner.id === "6666fc43f47c6f581caeb135"
                              ? "Me"
                          ...
                          <div
                            className={clsx(
                              " p-[10px] rounded-[5px] self-start flex flex-col gap-2 text-primary-2",
                              comment.owner.id === "6666fc43f47c6f581caeb135"
                                ? "bg-chats-2-default"
                                : "bg-chats-1-default"
                            )}
